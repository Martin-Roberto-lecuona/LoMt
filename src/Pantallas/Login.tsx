...
const Login = () => {
  ...
  const handleSubmit:React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      const response = await fetch(`${USERSLINK}?search=${user.username}`, {signal: controller.signal})
      if (!response.ok) 
      ...
      setUser({...user, mail: validUser[0].mail})/// Como hacer para que renderice el setUser antes que guardar en el localStorage
      
      if(checkedRemember){
        const aux = JSON.stringify(user)
        const  userData = JSON.parse(aux)
        userData.mail = validUser[0].mail /// arreglo temporal para guardar el mail en el localStorage
